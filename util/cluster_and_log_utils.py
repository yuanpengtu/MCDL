...
def split_cluster_acc_v2_label(y_true, y_pred, mask):
    """
    ...
    for i, f_name in enumerate(eval_funcs):
        ...
        if print_output:
            ...
            try:
                ...
                pass
    return to_return
def split_cluster_acc_v2_new(y_true, y_pred, mask):
    """
    Calculate clustering accuracy. Require scikit-learn installed
    First compute linear assignment on all data, then look at how good the accuracy is on subsets
    # Arguments
    ...
    for i in old_classes_gt:
        old_acc += w[ind_map[i], i]
        old_acc_all.append(w[ind_map[i], i]/30.0*100)
        total_old_instances += sum(w[:, i])
    ...
    for i in new_classes_gt:
        new_acc += w[ind_map[i], i]
        new_acc_all.append(w[ind_map[i], i]/30.0*100)
        total_new_instances += sum(w[:, i])
    ...
    try:
        ...
        pass
    new_acc /= total_new_instances
    print(old_classes_gt, old_acc_all)
    print(new_classes_gt, new_acc_all)
    return total_acc, old_acc, new_acc, ind_map, new_acc_all, old_acc_all
def split_cluster_acc_v2_balanced_new(y_true, y_pred, mask):
    """
    Calculate clustering accuracy. Require scikit-learn installed
    First compute linear assignment on all data, then look at how good the accuracy is on subsets
    # Arguments
    ...
    total_acc, old_acc, new_acc = total_acc.mean(), old_acc.mean(), new_acc.mean()
    return total_acc, old_acc, new_acc
def log_accs_from_preds_v2(y_true, y_pred, mask, eval_funcs, save_name, T=None,
                        print_output=True, args=None):
    """
    Given a list of evaluation functions to use (e.g ['v1', 'v2']) evaluate and log ACC results
    :param y_true: GT labels
    ...
    for i, f_name in enumerate(eval_funcs):
        if f_name=='v2b':
            acc_f = split_cluster_acc_v2_balanced_new
        else:
            acc_f =split_cluster_acc_v2_new 
        all_acc, old_acc, new_acc, ind_map,new_acc_all, old_acc_all = acc_f(y_true, y_pred, mask)
        log_name = f'{save_name}_{f_name}'
        if i == 0:
            to_return = (all_acc, old_acc, new_acc, ind_map,new_acc_all, old_acc_all)
        if print_output:
            print_str = f'Epoch {T}, {log_name}: All {all_acc:.4f} | Old {old_acc:.4f} | New {new_acc:.4f}'
            try:
                if dist.get_rank() == 0:
